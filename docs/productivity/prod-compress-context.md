## What it does

Summarizes active conversation history, decisions, and task state into a compact checkpoint.

## When to reach for it

You invoke this by typing `/prod-compress-context`, and the agent won't reach for it on its own.

You invoke this by typing /prod-compress-context, and the agent will not reach for it on its own.

## Common questions

**Does this skill work across multiple AI agent tools?**
Yes. It supports Claude Code, OpenAI Codex, DeepSeek Harness (DSH), and standard Agent Skills ecosystem tools.

## It's working if

- The skill executes with clear step-by-step progress and verifiable completion gates.
- Intermediate artifacts or inspection commands are visible and verified before moving to the next stage.

## Where it fits

Session maintenance tool for context compaction.
