...
function writePackage(root, pkg) {
  writeText(path.join(root, "package.json"), `${JSON.stringify(pkg, null, 2)}\n`);
}
...
function createNew(root) {
  if (!existsSync(templateRoot)) {
    throw new Error(`Bundled Adaptive Product Design prototype template is missing: ${templateRoot}`);
  }
...
function main() {
  ...
  const root = path.resolve(args.root || args.dest || process.cwd());

  if (args.mode && args.mode !== "new") {
    throw new Error(`Unknown mode: ${String(args.mode)}`);
  }

  console.log(JSON.stringify(createNew(root), null, 2));
}
